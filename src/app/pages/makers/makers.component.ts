...
export class MakersComponent implements OnInit {
          ...
          .subscribe((res) => {
            if (res["status"] == true) {
              if (currentTarget == 0)
                Swal.fire(
                  "Approved!",
                  "Selected Makers has been Enabled.",
                  "success"
                );
              else
                Swal.fire(
                  "Rejected!",
                  "Selected Makers has been Disabled.",
                  "success"
                );
              this._fetchData();
            }
  ...
  deleteRow(item) {
    Swal.fire({
      ...
      if (result.value) {
        this.authFackservice
          .delete("admin/parameter?id=" + item.id + "&type=makers")
          .subscribe((res) => {
            if (res["status"] == true) {
              Swal.fire(
                "Deleted!",
                "selected Maker has been deleted.",
                "success"
              );
              this._fetchData();
            }
  ...
  typeSubmit() {
    ...
    if (data.id == 0 || data.id == null) {
      this.authFackservice
        .postMultipart("admin/makers", formData)
        .subscribe((res) => {
          if (res["status"] == true) {
            this._fetchData();
      ...
      this.authFackservice
        .putMultipart("admin/makers", formData)
        .subscribe((res) => {
          if (res["status"] == true) {
            this._fetchData();
            Swal.fire(
              "Success!",
              "Selected Maker has been updated.",
              "success"
            );
          } else {
            Swal.fire("Error!", res["message"], "error");
