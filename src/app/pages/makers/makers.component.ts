...
keyword:string=''
  ...
  ngOnInit() {
    this.breadCrumbItems = [{label:'My Dashboard',href:'/dashboard'},{label:'Makers', active: true }];
    this._fetchData();
  ...
  public _fetchData() {
    this.authFackservice.get('admin/makers?page='+this.page.pageNumber+'&perPage=10&keyword='+this.keyword).subscribe(
      res => {
        if(res['status']==true){
          this.makersData =res['data'];
  ...
  Swal.fire({
    ...
    if (result.value) {
      this.authFackservice.put('admin/parameter/status?value='+currentTarget+'&id='+id+'&type=makers',{}).subscribe(
      res => {
        if(res['status']==true){
          if(currentTarget==0)
          Swal.fire('Approved!', 'Selected Makers has been Enabled.', 'success');
          else
          Swal.fire('Rejected!', 'Selected Makers has been Disabled.', 'success');
          this._fetchData();
        }
  ...
  deleteRow(item){
    Swal.fire({
      ...
      if (result.value) {
        this.authFackservice.delete('admin/parameter?id='+item.id+ '&type=makers').subscribe(
          res => {
            if(res['status']==true){
              Swal.fire('Deleted!', 'selected Maker has been deleted.', 'success');
              this._fetchData();
            }
          });  
      }
  ...
  typeSubmit() {
    ...
    formData.append("type", this.typeValidationForm.value.type);
    
    let data=this.typeValidationForm.value;
   if(data.id==0 || data.id==null){
      this.authFackservice.postMultipart('admin/makers',formData).subscribe(
        res => {
          if(res['status']==true){
            this._fetchData();
            Swal.fire('Success!', 'New Maker has been added.', 'success');
          }else{
      ...
      this.authFackservice.putMultipart('admin/makers',formData).subscribe(
        res => {
          if(res['status']==true){

            this._fetchData();
            Swal.fire('Success!', 'Selected Maker has been updated.', 'success');
