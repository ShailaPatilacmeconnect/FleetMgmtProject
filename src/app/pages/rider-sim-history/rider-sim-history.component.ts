...
keyword:string=''
  ...
  ngOnInit() {
    this.breadCrumbItems = [{label:'My Dashboard',href:'/dashboard'},{label:'Assign SIM', active: true }];
    this._fetchData();
  ...
  public _fetchData() {
    this.authFackservice.get('/admin/riders/simHistory?page='+this.page.pageNumber+'&perPage=10&keyword='+this.keyword).subscribe(
      res => {
        if(res['status']==true){
      ...
      this.authFackservice.get('/admin/riders').subscribe(
        ...
        this.authFackservice.get('/admin/mobileSims').subscribe(
          res => {
            if(res['status']==true){
  ...
  typeSubmit() {
    ...
    // console.log("mobile sim"+ this.typeValidationForm.value.name)
   ...
   if(data.id==0 || data.id==null){
      this.authFackservice.postMultipart('/admin/riders/simHistory',formData).subscribe(
        res => {
          if(res['status']==true){
            this._fetchData();
            Swal.fire('Success!', 'New MobileSimHistory has been added.', 'success');
          }else{
