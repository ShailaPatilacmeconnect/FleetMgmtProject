...
export class MobileSIMComponent implements OnInit {
  ...
  ngOnInit() {
    this.breadCrumbItems = [{label:'My Dashboard',href:'/dashboard'},{label:'Mobile SIM', active: true }];
    this._fetchData();
  ...
  initForm(){
    this.typeValidationForm = this.formBuilder.group({
      id:0,
      serial_number: ['', [Validators.required,Validators.maxLength(16), Validators.minLength(16)]],
  ...
  public _fetchData() {
    this.authFackservice.get('admin/mobileSims?page='+this.page.pageNumber+'&perPage=10&keyword='+this.keyword).subscribe(
      res => {
        if(res['status']==true){
  ...
  Swal.fire({
    ...
    if (result.value) {
      this.authFackservice.put('admin/parameter/status?value='+currentTarget+'&id='+id+'&type=mobile_sims',{}).subscribe(
      res => {
        if(res['status']==true){
          if(currentTarget==0)
          Swal.fire('Approved!', 'Selected MobileSIM has been Enabled.', 'success');
          else
          Swal.fire('Rejected!', 'Selected MobileSIM has been Disabled.', 'success');
          this._fetchData();
        }
      }); 
  ...
  deleteRow(item){
    Swal.fire({
      ...
      if (result.value) {
        this.authFackservice.delete('admin/parameter?id='+item.id+ '&type=mobile_sims').subscribe(
          res => {
            if(res['status']==true){
              Swal.fire('Deleted!', 'selected MobileSIM has been deleted.', 'success');
              this._fetchData();
            }
          });  
      }
  ...
  typeSubmit() {
    ...
    formData.append("serial_number", this.typeValidationForm.value.serial_number);
    formData.append("number", this.typeValidationForm.value.number);
    
    let data=this.typeValidationForm.value;
   if(data.id==0 || data.id==null){
      this.authFackservice.postMultipart('admin/mobileSims',formData).subscribe(
        res => {
          if(res['status']==true){
            this._fetchData();
      ...
      this.authFackservice.putMultipart('admin/mobileSims',formData).subscribe(
        res => {
          if(res['status']==true){

            this._fetchData();
            Swal.fire('Success!', 'Selected MobileSIM has been updated.', 'success');
