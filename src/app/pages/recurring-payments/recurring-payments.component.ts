...
export class RecurringPaymentsComponent implements OnInit {
  ...
  ngOnInit() {
    this.breadCrumbItems = [
      ...
      { label: "Recurring Payments", active: true },
    ];
    this._fetchData();
  }
  ...
  public _fetchData() {
    this.authFackservice
      ...
      .subscribe((res) => {
        if (res["status"] == true) {
          this.recurringPaymentsData = res["data"];
          ...
          .subscribe((res) => {
            if (res["status"] == true) {
              if (currentTarget == 0)
                Swal.fire(
                  "Approved!",
                  "Selected Recurring Payment has been Enabled.",
                  "success"
                );
              else
                Swal.fire(
                  "Rejected!",
                  "Selected Recurring Payment has been Disabled.",
                  "success"
                );
              this._fetchData();
            }
  ...
  deleteRow(item) {
    Swal.fire({
      ...
      if (result.value) {
        this.authFackservice
          .delete("admin/parameter?id=" + item.id + "&type=recurring_payments")
          .subscribe((res) => {
            if (res["status"] == true) {
              Swal.fire(
                "Deleted!",
                "selected Recurring Payment has been deleted.",
                "success"
              );
              this._fetchData();
            }
  ...
  typeSubmit() {
    ...
    if (data.id == 0 || data.id == null) {
      this.authFackservice
        .postMultipart("admin/recurringPayments", formData)
        .subscribe((res) => {
          if (res["status"] == true) {
            this._fetchData();
            Swal.fire(
              "Success!",
              "New Recurring Payment has been added.",
              "success"
            );
          } else {
      ...
      this.authFackservice
        .putMultipart("admin/recurringPayments", formData)
        .subscribe((res) => {
          if (res["status"] == true) {
            this._fetchData();
            Swal.fire(
              "Success!",
              "Selected Recurring Payment has been updated.",
              "success"
            );
          } else {
            Swal.fire("Error!", res["message"], "error");
