...
keyword:string=''
  ...
  ngOnInit() {
    this.breadCrumbItems = [{label:'My Dashboard',href:'/dashboard'},{label:'Assign Car', active: true }];
    this._fetchData();
  ...
  public _fetchData() {
    this.authFackservice.get('admin/rideHistory/car?page='+this.page.pageNumber+'&perPage=10&keyword='+this.keyword).subscribe(
      res => {
        if(res['status']==true){
      ...
      this.authFackservice.get('/admin/riders').subscribe(
        ...
        this.authFackservice.get('/admin/cars').subscribe(
          res => {
            if(res['status']==true){
  ...
  typeSubmit() {
   ...
   if(data.id==0 || data.id==null){
      this.authFackservice.postMultipart('admin/rideHistory/car',formData).subscribe(
        res => {
          if(res['status']==true){
            this._fetchData();
            Swal.fire('Success!', 'New RiderCarHistory has been added.', 'success');
          }else{
      ...
      this.authFackservice.putMultipart('admin/rideHistory/car',formData).subscribe(
        res => {
          if(res['status']==true){

            this._fetchData();
            Swal.fire('Success!', 'Selected RiderCarHistory has been updated.', 'success');
