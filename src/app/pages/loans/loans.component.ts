...
keyword:string=''
  ...
  ngOnInit() {
    this.breadCrumbItems = [{label:'My Dashboard',href:'/dashboard'},{label:'Loans', active: true }];
    this._fetchData();
  ...
  public _fetchData() {
    this.authFackservice.get('admin/loans?page='+this.page.pageNumber+'&perPage=10&keyword='+this.keyword).subscribe(
      res => {
        if(res['status']==true){
  ...
  Swal.fire({
    ...
    if (result.value) {
      this.authFackservice.put('admin/parameter/status?value='+currentTarget+'&id='+id+'&type=loans',{}).subscribe(
      res => {
        if(res['status']==true){
          if(currentTarget==0)
          Swal.fire('Approved!', 'Selected Loan has been Enabled.', 'success');
          else
          Swal.fire('Rejected!', 'Selected Loan has been Disabled.', 'success');
          this._fetchData();
        }
  ...
  deleteRow(item){
    Swal.fire({
      ...
      if (result.value) {
        this.authFackservice.delete('admin/parameter?id='+item.id+ '&type=loans').subscribe(
          res => {
            if(res['status']==true){
              Swal.fire('Deleted!', 'selected Loan has been deleted.', 'success');
              this._fetchData();
            }
          });  
      }
  ...
  typeSubmit() {
   ...
   if(data.id==0 || data.id==null){
      this.authFackservice.postMultipart('admin/loans',formData).subscribe(
        res => {
          if(res['status']==true){
            this._fetchData();
      ...
      this.authFackservice.putMultipart('admin/loans',formData).subscribe(
        res => {
          if(res['status']==true){
